...
def plot_dmd_fft_comparison(
    ...
    # Get REAL time vector for this Re
...
def plot_dmd_reconstruction_error(
    ...
    ax.text(0.98, 0.95, f"Mean Error = {mean_error_train:.2f}%",
...
def plot_dmd_forecast_error(
    ...
    ax.set_xlabel(r"$t^* = t U_{ref} / L_{ref}$", fontsize=13)
    ax.set_ylabel(r"Relative $L^2$ Error", fontsize=13)
    ax.set_title(f"ParametricDMD Forecast Error\n $Training$ Parameter $(Re = {Re_target})$", fontsize=15)
    ax.grid(True, alpha=0.6)
...
def plot_dmd_modal_comparison_interp_vs_true(
    ...
    for mode in range(n_modes_to_plot):
        ...
        ax.set_ylabel("Amplitude", fontsize=12)
        ax.set_title(f"Mode $\\boldsymbol{{\\psi_{{{mode+1}}}}}$",
                     fontsize=13, pad=6)
        ax.grid(alpha=0.6)
        ax.legend(fontsize=11, loc="upper right")
        ax.set_xlim(time_nd[0], time_nd[-1])
...
def plot_dmd_fft_comparison_interp_vs_true(
    ...
    # Sampling times for this Re
    times = np.array([float(t) for t in times_test], dtype=float)

    # Automatic dt from CFD time vector
    dt = times[1] - times[0]
    ...
    # Frequency axis (non-dimensional Strouhal number)
    freqs = np.fft.rfftfreq(modal_true.shape[1], d=dt)
    St = freqs * L_ref / U_ref
    ...
    for mode in range(n_modes_to_plot):
        ...
        true_mode = modal_true[mode].real
        dmd_mode  = modal_dmd[mode].real

        fft_true  = np.abs(np.fft.rfft(true_mode))
        fft_dmd   = np.abs(np.fft.rfft(dmd_mode))
...
def plot_flow_comparison_interpolated_dmd_vs_true(
    ...
    for idx_t, idx_i in zip(true_indices, interp_indices):
        ...
        mag_interp_all.append(mag_i)

    mag_true_all = np.concatenate(mag_true_all)
    mag_interp_all = np.concatenate(mag_interp_all)
...
def plot_interp_reconstruction_error(snapshot_test,
    ...
    U_ref = Re_test * nu / L_ref
    t_star = (t_true - t_true[0]) * U_ref / L_ref

    # Relative L2 error
    abs_err = np.linalg.norm(X_true - X_interp, axis=0)
    rel_err = abs_err / np.linalg.norm(X_true, axis=0)
    ...
    ax.set_ylabel(r"Relative $L^2$ Error", fontsize=13)
    ax.set_title(
        r"ParametricDMD Reconstruction Error"
        f"\n $Unseen$ Parameter $(Re = {Re_test})$",
        fontsize=15
    )
    ax.grid(True, alpha=0.6)
